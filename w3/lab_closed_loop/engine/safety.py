"""Blast-radius guard and circuit breaker."""
...
class BlastRadiusGuard:
    ...
    def check(self, service: str) -> tuple[bool, str]:
        ...
        if len(self._global_window) >= self._max_per_minute:
            return False, f"global limit ({self._max_per_minute} actions/min) reached"
        if len(self._service_window[service]) >= self._max_restarts_per_hour:
            return False, f"per-service limit ({self._max_restarts_per_hour}/hr) for {service}"
        return True, "ok"
    ...
    def record_failure(self):
        ...
        if self._failures >= self._threshold:
            ...
            log.error(
                ...
                consecutive_failures=self._failures,
                threshold=self._threshold,
                message="Automation halted. Manual reset required.",
            )
