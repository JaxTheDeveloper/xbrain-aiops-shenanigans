...
def evaluate_holdout_performance(model, labeled_df: pd.DataFrame, name: str):
    """Evaluate a model on labeled holdout data and handle degenerate holdout label mixes."""
    X_hold = labeled_df[FEATURES].dropna()
    y_true = labeled_df.loc[X_hold.index, "anomaly_label"].values

    raw_preds = model.predict(X_hold)
    ...
    if positive_count > 0:
        ...
        if negative_count > 0:
            ...
            extra = f" (false positive rate: {false_positive_rate:.4f})"
    elif negative_count > 0:
        false_positive_rate = fp / negative_count
        extra = (
            " (no positive labels in holdout; precision/recall undefined; "
            f"false positive rate: {false_positive_rate:.4f})"
        )
    else:
        extra = " (holdout has no labeled rows after filtering)"

    print(
        f"[retrain] Holdout validation — {name} precision: "
        f"{precision if precision is not None else 'N/A'} "
        f"recall: {recall if recall is not None else 'N/A'}{extra}"
    )
    return {
        "precision": precision,
        "recall": recall,
    ...
    for cycle in range(1, cycles + 1):
        ...
        if precision < prec_threshold:
            ...
            try:
                ...
                print(f"[post_deploy_monitor] ERROR: rollback alias swap failed: {exc}")

            reload_ok = reload_serve(serve_url)
            if rollback_ok:
                print(f"[post_deploy_monitor] Rollback complete. v{v1_version} restored to @production. v{v2_version} → @archived.")
            else:
                print(f"[post_deploy_monitor] Rollback failed. v{v1_version} may not be restored to @production.")
            ...
            append_audit("auto_rollback_v2_to_v1", {
                ...
                "rollback_alias_success": rollback_ok,
...
def main():
    ...
    # Validate v2 on holdout (old pattern) — must not be worse than current production version.
    if args.holdout:
        mlflow.set_tracking_uri(tracking_uri)
        holdout_df = pd.read_csv(args.holdout)
        if "anomaly_label" in holdout_df.columns:
            v1_results = evaluate_holdout_performance(
                mlflow.pyfunc.load_model(f"models:/{MODEL_NAME}@production"), holdout_df, "v1"
            )
            v2_results = evaluate_holdout_performance(model, holdout_df, "v2")
            append_audit(
                "holdout_validation",
                {
                    "v1_precision": v1_results["precision"],
                    "v1_recall": v1_results["recall"],
                    "v1_false_positive_rate": v1_results["false_positive_rate"],
                    "v1_positive_count": v1_results["positive_count"],
                    "v1_negative_count": v1_results["negative_count"],
                    "v2_precision": v2_results["precision"],
                    "v2_recall": v2_results["recall"],
                    "v2_false_positive_rate": v2_results["false_positive_rate"],
                    "v2_positive_count": v2_results["positive_count"],
                    "v2_negative_count": v2_results["negative_count"],
                },
            )
